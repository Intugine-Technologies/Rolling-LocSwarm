  ...
  if (
    ...
    if (in_geofence(last_halt.loc, latest_ping.loc, distance_threshold)) {
      ...
      halts[halts.length - 1].duration =
        Date.parse(latest_ping.time) - Date.parse(last_halt.from_time);
    }
  } else if (last_ping) {
    if (in_geofence(last_ping.loc, latest_ping.loc, distance_threshold)) {
      halts.push({
        ...
        duration: Date.parse(latest_ping.time) - Date.parse(last_ping.time),
        loc: last_ping.loc,
      });
    }
  }
  return halts.filter((x) => x.duration > 0);
};
